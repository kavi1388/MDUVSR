...
import sys
sys.path.append('/content/drive/MyDrive/VSR/ddf/src')
import ddf_mul_ext, ddf_mul_faster_ext, ddf_add_ext, ddf_add_faster_ext
...
class DDFPack(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=1, nonlinearity='relu', kernel_combine='mul'):
...
class DDFUpPack(nn.Module):
    def __init__(self, in_channels, kernel_size=3, scale_factor=2, dilation=1, head=1, se_ratio=1,
    ...
    def forward(self, x, joint_x=None):
        ...
        for s_b, c_b in zip(self.spatial_branch, self.channel_branch):
            ...
            o = ddf(_x, channel_filter, spatial_filter,
                    self.kernel_size, self.dilation, 1, self.kernel_combine).type_as(x)  #removed self.head from here
